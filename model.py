...
class Web_Detector(Detector):  # 定义YOLOv8Detector类，继承自Detector类
    ...
    def load_model(self, model_path):  # 定义加载模型的方法
        ...
        if os.path.basename(model_path).startswith('yolov8'):
            task = 'segment'
        else:
            task = 'detect' # 默认设置为detect，但seg模型通常是segment

        self.model = YOLO(model_path, task=task)
        # 核心修改：将YOLO模型自带的英文/数字类别名称映射为中文
        # self.model.names 是一个字典，键是类别ID（数字字符串），值是原始类别名称（通常是英文）
        mapped_names = {}
        for idx, original_name in self.model.names.items():
            if original_name in Chinese_name: # 尝试用原始英文名称作为key
                mapped_names[idx] = Chinese_name[original_name]
            elif str(idx) in Chinese_name: # 或者尝试用类别ID的字符串形式作为key
                mapped_names[idx] = Chinese_name[str(idx)]
            else: # 如果都没有匹配到，则使用原始名称 (这表示该类别没有中文映射)
                mapped_names[idx] = original_name

        self.names = [mapped_names[i] for i in sorted(mapped_names.keys(), key=int)]

        input_imgsz = getattr(self.model, 'imgsz', 640) # 尝试从模型获取imgsz，否则默认640
        self.model(torch.zeros(1, 3, *[input_imgsz] * 2).to(self.device).
